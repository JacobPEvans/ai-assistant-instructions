...
> **Purpose**: This command manages YOUR pull requests as the PR AUTHOR - creating, monitoring, fixing, and preparing them for merge. For reviewing OTHER people's PRs, use `/rok-review-pr`.
...
> **Usage:**
>
> **Command Execution**: `/rok-manage-pr`
>
> **Example Prompts**:
>
...
- **Issue context**: `/rok-shape-issues` and `/rok-resolve-issues` provide background for PR creation
- **Review phase**: Others use `/rok-review-pr` to review your PR
- **Feedback resolution**: Use `/rok-resolve-pr-review-thread` for efficient review thread resolution

**Complete Development Lifecycle**: `/init-worktree` -> `/rok-resolve-issues` -> `/rok-manage-pr` -> (reviewer uses `/rok-review-pr`) -> `/rok-resolve-pr-review-thread` -> merge
