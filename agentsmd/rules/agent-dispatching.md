...
- Search file contents with the Grep tool (NEVER grep, rg via Bash)
...
Subagents using `sed`/`awk`/`python -c` for file edits is the #1 quality issue — enforce this strictly.
